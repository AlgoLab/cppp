  ...
  [2*n/4, 3*n/4, 4*n/4, 5*n/4, 6*n/4].each do |m|
    ...
    100.times do |i|
      unless File.exists?("#{dirname}/#{i}.ms")
        ...
        system "/usr/bin/time -f \"%e\" -o #{dirname}/#{i}.log timeout -s 9 30m ./cppp  #{dirname}/#{i}.data > #{dirname}/#{i}.out"
      end
        ...
        if i < 10
          10.times do |x|
            unless File.exists?("#{mod_dirname}/#{x}.data")
              ...
              system "/usr/bin/time -f \"%e\" -o #{mod_dirname}/#{x}.log timeout -s 9 30m ./cppp  #{mod_dirname}/#{x}.data > #{mod_dirname}/#{x}.out"
            end
          end
        end
      end
